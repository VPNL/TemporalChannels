function obj_fun = tch_obj_fun_2ch_exp_quad_opt(roi, model)
...
% sustained and quadratic transient channels).
% 
% INPUTS:
...
if ~strcmp(model.type, '2ch-exp-quad-opt'); error('Incompatible model type'); end
...
conv_nb = @(s, tau, tau_ae) cellfun(@(S, T) [S T], ...
    conv_nbS(s, tau, tau_ae), conv_nbT(s, tau), 'uni', false);
