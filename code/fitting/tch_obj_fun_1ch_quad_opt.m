function obj_fun = tch_obj_fun_1ch_quad_opt(roi, model)
% Generates anonymous objective function that can be passed to fmincon for
% the 1ch-quad-opt model (optimized 1-channel model with quadratic 
% transient channels).
% 
% INPUTS:
...
if ~strcmp(model.type, '1ch-quad-opt'); error('Incompatible model type'); end
...
conv_nb = @(s, tau) cellfun(@(T) [T], ...
    conv_nbT(s, tau), 'uni', false);
...
comp_ws = @(s, tau, m, b0) cell2mat(conv_nb(s, tau)) \ cell2mat(comp_bs(m, b0));
% predicted signal: channel predictors x channel weights
pred_bs = @(s, tau, m, b0) cellfun(@(P, W) P .* repmat(W, size(P, 1), 1), ...
    conv_nb(s, tau), repmat({comp_ws(s, tau, m, b0)'}, nruns, 1), 'uni', false);
% model residuals: (predicted signal - measured signal)^2
calc_br = @(s, tau, m, b0) cellfun(@(S, M) (sum(S, 2) - M) .^ 2, ...
    pred_bs(s, tau, m, b0), comp_bs(m, b0), 'uni', false);
% model error: summed squared residuals for all run time series
calc_me = @(s, tau, m, b0) sum(cell2mat(calc_br(s, tau, m, b0)));
