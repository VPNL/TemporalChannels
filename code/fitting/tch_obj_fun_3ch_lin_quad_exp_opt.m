...
delay_fun = @(d) exp(-(1:12000) / (d * 1000));
...
conv_snD = @(s, d) cellfun(@(X, Y, ON, OFF) code_exp_decay(X, ON, OFF, Y, fs), ...
    cellfun(@code_delay_act, s, 'uni', false), repmat({delay_fun(d)}, nruns, 1), ...
...
conv_nbD = @(s, d) cellfun(@(ND) convolve_vecs(ND, irfs.hrf{1}, fs, 1 / tr), ...
    conv_snD(s, d), 'uni', false);
% channel predictors: [sustained BOLD, transient BOLD, delay BOLD]
conv_nb = @(s, x, y, z, d) cellfun(@(S, T, D) [S T D], ...
    conv_nbS(s, x, z), conv_nbT(s, y, z), conv_nbD(s, d), 'uni', false);
...
comp_ws = @(s, x, y, z, d, m, b0) cell2mat(conv_nb(s, x, y, z, d)) \ cell2mat(comp_bs(m, b0));
% predicted signal: channel predictors x channel weights
pred_bs = @(s, x, y, z, d, m, b0) cellfun(@(P, W) P .* repmat(W, size(P, 1), 1), ...
    conv_nb(s, x, y, z, d), repmat({comp_ws(s, x, y, z, d, m, b0)'}, nruns, 1), 'uni', false);
% model residuals: (predicted signal - measured signal)^2
calc_br = @(s, x, y, z, d, m, b0) cellfun(@(S, M) (sum(S, 2) - M) .^ 2, ...
    pred_bs(s, x, y, z, d, m, b0), comp_bs(m, b0), 'uni', false);
% model error: summed squared residuals for all run time series
calc_me = @(s, x, y, z, d, m, b0) sum(cell2mat(calc_br(s, x, y, z, d, m, b0)));
