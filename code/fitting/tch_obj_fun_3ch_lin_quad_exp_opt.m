...
conv_snT = @(s, tau, n1, n2, k) cellfun(@(X, Y) convolve_vecs(X, Y, 1, 1) .^ 2, ...
...
comp_ws = @(s, tau, n1, n2, k, tau_pe, m, b0) cell2mat(conv_nb(s, tau, n1, n2, k, tau_pe)) \ cell2mat(comp_bs(m, b0));
% predicted signal: channel predictors x channel weights
pred_bs = @(s, tau, n1, n2, k, tau_pe, m, b0) cellfun(@(P, W) P .* repmat(W, size(P, 1), 1), ...
    conv_nb(s, tau, n1, n2, k, tau_pe), repmat({comp_ws(s, tau, n1, n2, k, tau_pe, m, b0)'}, nruns, 1), 'uni', false);
