function obj_fun = tch_obj_fun_2ch_exp_crect(roi, model)
% Generates anonymous objective function that can be passed to fmincon for
% 2-channel model with optimized adapated sustained and rectified + 
...
if ~strcmp(model.type, '2ch-exp-crect'); error('Incompatible model type'); end
...
conv_snTr = @(s, tau) cellfun(@(X, Y) rectify(convolve_vecs(X, Y, 1, 1)), ...
    s, repmat({nrfT_fun(tau)}, nruns, 1), 'uni', false);
conv_snTc = @(s, tau, epsilon) cellfun(@(X, Y) rectify(X, 'abs', .001) .^ Y, ...
    conv_snTr(s, tau), repmat({epsilon}, nruns, 1), 'uni', false);
