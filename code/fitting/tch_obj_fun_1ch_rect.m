function obj_fun = tch_obj_fun_1ch_rect(roi, model)
% Generates anonymous objective function that can be passed to fmincon for
% the 1ch-rect model (optimized 1-channel model with rectified transient 
% channels).
% 
% INPUTS:
...
if ~strcmp(model.type, '1ch-rect'); error('Incompatible model type'); end
...
conv_snT = @(s, tau_t) cellfun(@(X, Y) rectify(convolve_vecs(X, Y, 1, 1)), ...
