function obj_fun = tch_obj_fun_2ch_exp_quad(roi, model)
% Generates anonymous objective function that can be passed to fmincon for
% the 2ch-exp-quad model (2-channel model with adapted sustained and
...
if ~strcmp(model.type, '2ch-exp-quad'); error('Incompatible model type'); end
...
conv_snT = @(x) cellfun(@(X) convolve_vecs(X, irfs.nrfT{1}, 1, 1) .^ 2, ...
    x, 'uni', false);
% susatined BOLD: sustained response * HRF
