function obj_fun = tch_obj_fun_1ch_lin(roi, model)
% Generates anonymous objective function that can be passed to fmincon for
% the 1ch-lin model (optimized model with linear sustained channels).
% 
% INPUTS:
...
if ~strcmp(model.type, '1ch-lin'); error('Incompatible model type'); end
...
comp_ws = @(s, tau_s, m, b0) cell2mat(conv_nbS(s, tau_s)) \ cell2mat(comp_bs(m, b0));
% predicted signal: channel predictors x channel weights
pred_bs = @(s, tau_s, m, b0) cellfun(@(P, W) P .* repmat(W, size(P, 1), 1), ...
    conv_nbS(s, tau_s), repmat({comp_ws(s, tau_s, m, b0)'}, nruns, 1), 'uni', false);
