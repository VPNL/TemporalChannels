...
conv_snTs = @(s, tau_t, Lp, Kp, Ln, Kn) cellfun(@(X, lp, kp, ln, kn) tch_sigmoid(X, lp, kp, ln, kn), ...
    conv_snT(s, tau_t), repmat({Lp}, nruns, 1), repmat({Kp}, nruns, 1), repmat({Ln}, nruns, 1), repmat({Kn}, nruns, 1), 'uni', false);
% transient BOLD: transient response * HRF
conv_nbT = @(s, tau_t, Lp, Kp, Ln, Kn) cellfun(@(NT) convolve_vecs(NT, irfs.hrf{1}, fs, 1 / tr), ...
    conv_snTs(s, tau_t, Lp, Kp, Ln, Kn), 'uni', false);
...
comp_ws = @(s, tau_t, Lp, Kp, Ln, Kn, m, b0) ...
    cell2mat(conv_nbT(s, tau_t, Lp, Kp, Ln, Kn)) \ cell2mat(comp_bs(m, b0));
% predicted signal: channel predictors x channel weights
pred_bs = @(s, tau_t, Lp, Kp, Ln, Kn, m, b0) cellfun(@(P, W) P .* repmat(W, size(P, 1), 1), ...
    conv_nbT(s, tau_t, Lp, Kp, Ln, Kn), repmat({comp_ws(s, tau_t, Lp, Kp, Ln, Kn, m, b0)'}, nruns, 1), 'uni', false);
% model residuals: (predicted signal - measured signal)^2
calc_br = @(s, tau_t, Lp, Kp, Ln, Kn, m, b0) cellfun(@(S, M) (sum(S, 2) - M) .^ 2, ...
    pred_bs(s, tau_t, Lp, Kp, Ln, Kn, m, b0), comp_bs(m, b0), 'uni', false);
% model error: summed squared residuals for all run time series
calc_me = @(s, tau_t, Lp, Kp, Ln, Kn, m, b0) ...
    sum(cell2mat(calc_br(s, tau_t, Lp, Kp, Ln, Kn, m, b0)));
