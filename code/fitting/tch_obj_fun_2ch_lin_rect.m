function obj_fun = tch_obj_fun_2ch_lin_rect(roi, model)
% Generates anonymous objective function that can be passed to fmincon for
% the 2ch-lin-rect model (optimized 2-channel model with sustained and  
...
if ~strcmp(model.type, '2ch-lin-rect'); error('Incompatible model type'); end
...
% transient response: rectify(stimulus * transient IRF)
conv_snT = @(s, tau_s) cellfun(@(X, Y) rectify(convolve_vecs(X, Y, 1, 1)), ...
