function obj_fun = tch_obj_fun_3ch_exp_cquad_rect(roi, model)
% Generates anonymous objective function that can be passed to fmincon for
% 3-channel model with optimized adapated sustained and quadratic + 
% compressed transient and persistent channels).
% 
% INPUTS:
...
if ~strcmp(model.type, '3ch-exp-cquad-rect'); error('Incompatible model type'); end
...
% persistent response: max(0, stimulus * persistent IRF)
conv_snP = @(s, tau_p) cellfun(@(X, Y) rectify(convolve_vecs(X, Y, 1, 1)) .^ 2, ...
    s, repmat({nrfP_fun(tau_p)}, nruns, 1), 'uni', false);
...
conv_nbP = @(s, tau_p) cellfun(@(NT) convolve_vecs(NT, irfs.hrf{1}, fs, 1 / tr), ...
    conv_snP(s, tau_p), 'uni', false);
% channel predictors: [sustained BOLD, transient BOLD, persistent BOLD]
conv_nb = @(s, tau_s, tau_t, tau_p, tau_ae) cellfun(@(S, T, P) [S T P], ...
    conv_nbS(s, tau_s, tau_ae), conv_nbT(s, tau_t), conv_nbP(s, tau_p), 'uni', false);
...
comp_ws = @(s, tau_s, tau_t, tau_p, tau_ae, m, b0) ...
    cell2mat(conv_nb(s, tau_s, tau_t, tau_p, tau_ae)) \ cell2mat(comp_bs(m, b0));
% predicted signal: channel predictors x channel weights
pred_bs = @(s, tau_s, tau_t, tau_p, tau_ae, m, b0) cellfun(@(P, W) P .* repmat(W, size(P, 1), 1), ...
    conv_nb(s, tau_s, tau_t, tau_p, tau_ae), repmat({comp_ws(s, tau_s, tau_t, tau_p, tau_ae, m, b0)'}, nruns, 1), 'uni', false);
% model residuals: (predicted signal - measured signal)^2
calc_br = @(s, tau_s, tau_t, tau_p, tau_ae, m, b0) cellfun(@(S, M) (sum(S, 2) - M) .^ 2, ...
    pred_bs(s, tau_s, tau_t, tau_p, tau_ae, m, b0), comp_bs(m, b0), 'uni', false);
% model error: summed squared residuals for all run time series
calc_me = @(s, tau_s, tau_t, tau_p, tau_ae, m, b0) ...
    sum(cell2mat(calc_br(s, tau_s, tau_t, tau_p, tau_ae, m, b0)));
