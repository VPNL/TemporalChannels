...
pred_bsS = @(x, b) cellfun(@(PS, BS) PS .* repmat(BS, size(PS, 1), 1), ...
    conv_nbS(x), repmat({b}, nruns, 1), 'uni', false);
pred_bsT = @(x, b) cellfun(@(PT, BT) PT .* repmat(BT, size(PT, 1), 1), ...
    conv_nbT(x), repmat({b}, nruns, 1), 'uni', false);
pred_bsD = @(x, y, b) cellfun(@(PD, BD) PD .* repmat(BD, size(PD, 1), 1), ...
    conv_nbD(x, y), repmat({b}, nruns, 1), 'uni', false);
comp_bs = @(m, b0) cellfun(@(M, B0) M - repmat(B0, size(M, 1), 1), ...
    m, b0, 'uni', false);
calc_br = @(x, y, b, m, b0) cellfun(@(SS, ST, SD, M) (sum([SS ST SD], 2) - M) .^ 2, ...
    pred_bsS(x, b(1:npreds)), pred_bsT(x, b([1:npreds] + npreds)), ...
    pred_bsD(x, y, b([1:npreds] + 2 * npreds)), comp_bs(m, b0), 'uni', false);
calc_me = @(x, y, b, m, b0) sum(cell2mat(calc_br(x, y, b, m, b0)));
obj_fun = @(x) calc_me(stim, x(1), x([1:npreds * 3] + nparams), run_avgs, baseline);
