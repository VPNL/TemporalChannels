...
% quadratic transient, and exponential persistent channels).
% 
% INPUTS:
...
persist_fun = @(y) exp(-(1:12000) / (y * 1000));
...
conv_snP = @(x, y) cellfun(@(X, Y, ON, OFF) code_exp_decay(X, ON, OFF, Y, fs), ...
    cellfun(@code_persist_act, x, 'uni', false), repmat({persist_fun(y)}, nruns, 1), ...
...
conv_nbP = @(x, y) cellfun(@(NP) convolve_vecs(NP, irfs.hrf{1}, fs, 1 / tr), ...
    conv_snP(x, y), 'uni', false);
% channel predictors: [sustained BOLD, transient BOLD, persistent BOLD]
conv_nb = @(x, y) cellfun(@(S, T, P) [S T P], ...
    conv_nbS(x), conv_nbT(x), conv_nbP(x, y), 'uni', false);
