function obj_fun = tch_obj_fun_1ch_div(roi, model)
% Generates anonymous objective function that can be passed to fmincon for
% the 1ch-div model (single channel with CTS with divisive normalization)
% 
% INPUTS:
...
if ~strcmp(model.type, '1ch-div'); error('Incompatible model type'); end
...
% nonlinear response: (stim * IRF[tau1])^2
conv_sn = @(x, y) cellfun(@(X, Y) convolve_vecs(X, Y, 1, 1) .^ 2, ...
    x, repmat({irf_fun(y)}, nruns, 1), 'uni', false);
% neural response: nonlinear response / (nonlinear response + sigma^2)
comp_dn = @(x, y, z) cellfun(@(N, Z) N ./ (N + Z .^ 2), ...
    conv_sn(x, y), repmat({z}, nruns, 1), 'uni', false);
% bold response: neural response * HRF
conv_nb = @(x, y, z) cellfun(@(N) convolve_vecs(N, irfs.hrf{1}, fs, 1 / tr), ...
    comp_dn(x, y, z), 'uni', false);
