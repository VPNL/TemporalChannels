...
conv_snS = @(s, tau, n1, n2, k) ...
    cellfun(@(X, Y) convolve_vecs(X, Y, 1, 1), ...
    s, repmat({nrfS_fun(tau, n1, n2, k)}, nruns, 1), 'uni', false);
% transient response: (stimulus * transient IRF)^2
conv_snT = @(s, tau, n1, n2, k) cellfun(@(X, Y) ...
    convolve_vecs(X, Y, 1, 1) .^ 2, ...
    s, repmat({nrfT_fun(tau, n1, n2, k)}, nruns, 1), 'uni', false);
% sustained BOLD: sustained response * HRF
conv_nbS = @(s, tau, n1, n2, k) ...
    cellfun(@(NS) convolve_vecs(NS, irfs.hrf{1}, fs, 1 / tr), ...
    conv_snS(s, tau, n1, n2, k), 'uni', false);
% transient BOLD: transient response * HRF
conv_nbT = @(s, tau, n1, n2, k) ...
    cellfun(@(NT) convolve_vecs(NT, irfs.hrf{1}, fs, 1 / tr), ...
