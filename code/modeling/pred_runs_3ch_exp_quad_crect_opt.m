function model = pred_runs_3ch_exp_quad_crect_opt(model)
% Generates run predictors using the 3 temporal-channel model with 
% optimized adapted sustained, quadratic transient, and
% rectified/compressed persistent channel
...
predPc = cellfun(@(X, Y, Z) rectify(convolve_vecs(X, Y, fs, fs)) .^ Z, ...
    stim, irfs.nrfP, params.epsilon , 'uni', false); predPc(empty_cells) = {1};
...
fmriP = cellfun(@(X, Y) convolve_vecs(X, Y, fs, 1 / tr), ...
    predPc, irfs.hrf, 'uni', false); fmriP(empty_cells) = {[]};
