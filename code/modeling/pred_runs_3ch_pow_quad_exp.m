function model = pred_runs_3ch_pow_quad_exp(model)
% Generates run predictors using the 3 temporal-channel model with CTS-p on
% sustained, quradratic transient, and optimized persistent channel.
...
predSc = cellfun(@(X, Y) X .^ Y, ...
    predS, repmat(model.params.epsilon, nruns_max, 1), 'uni', false);
...
fmriS = cellfun(@(X, Y) convolve_vecs(X, Y, fs, 1 / tr), ...
    predSc, irfs.hrf, 'uni', false); fmriS(empty_cells) = {[]};
