function model = pred_runs_cts_pow(model)
% Generates run predictors using the CTS-pow model proposed by Zhou et al. 
% (2017).
...
predS = cellfun(@(X, Y, Z) convolve_vecs(X, Y, fs, fs) .^ Z, stim, irfs.nrfS, params.epsilon, 'uni', false);
predS(empty_cells) = {[]};
