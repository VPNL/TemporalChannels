function model = pred_trials_2ch_exp_dquad(model)
% Generates trial predictors using the 2 temporal-channel model with 
% adapted sustained and quadratic + normalized transient channels.
...
for ee = 1:nexps
    ...
    for cc = 1:length(cond_list{ee})
        ...
        for ss = 1:length(sessions)
            ...
            predTq = rectify(convolve_vecs(cstim, irfs.nrfT{ss}, fs, fs) .^ 2, 'abs', .001);
            predTd = predTq ./ (predTq + model.params.sigma{ss} .^ 2);
            % convolve neural predictors with HRF
            fmriS = convolve_vecs(adapt_act, irfs.hrf{ss}, fs, 1 / tr);
            fmriT = convolve_vecs(predTd, irfs.hrf{ss}, fs, 1 / tr);
