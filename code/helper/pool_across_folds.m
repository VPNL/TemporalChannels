...
for ee = 1:length(roi1)
    ...
    roi(ee).predD = cellfun(@(X, Y) (X + Y) ./ 2, ...
        roi1(ee).predD, roi2(ee).predD, 'uni', false);
    if strcmp(roi(ee).model.type, '3ch')
        roi(ee).pred = cellfun(@(X, Y, Z) X + Y + Z, ...
            roi(ee).predS, roi(ee).predT, roi(ee).predD, 'uni', false);
    elseif sum(strcmp(roi(ee).model.type(1:3), {'2ch' 'htd'}))
